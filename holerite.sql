WITH FOL AS (SELECT VRS.NUMCAD, VRS.CODEVE, EVC.DESEVE, EVC.TIPEVE, 
   ...
   JOIN VETORH.R026INF BIN ON (BIN.PERREF = (SELECT MAX(AX.PERREF) FROM VETORH.R026INF AX WHERE AX.PERREF <= :PERREF_PARAM ) AND BIN.CODFAI = 4)
   ...
   LEFT JOIN VETORH.R054CTD CTD ON (CTD.NUMEMP  = VRS.NUMEMP) AND (CTD.NUMCAD = VRS.NUMCAD) AND (CTD.CMPCTD = CAL.PERREF) AND (CTD.TIPCOL = VRS.TIPCOL)
