WITH FOL AS (SELECT VRS.NUMCAD, VRS.CODEVE, EVC.DESEVE, EVC.TIPEVE, 
        ...
        SAL.VALSAL, 
        CASE WHEN CAL.TIPCAL in (11) THEN CTD.TOTREM END AS TOTREM,
        INC.INCIRM
...
SELECT EMP.RAZSOC, FUN.DATADM, FUN.NUMCAD, FUN.NOMFUN AS NOME, FUN.NUMCPF, FOL.CODEVE, FOL.DESEVE, FOL.TIPEVE,
       ...
       FOL.VALSAL "SALBASE",
       FOL.BASEINSS,
       case when nvl(FOL.TOTREM,0) = 0 then FOL.BASEINSS else FOL.TOTREM + nvl((SELECT SUM(NVL(AX.VAL,0)) FROM FOL AX WHERE AX.CODEVE = 220),0) end AS BASFGTS,
       --case when FOL.BASEIR = 0 then FOL.BASEINSS else FOL.BASEIR end AS BASEIR,
       (SELECT SUM(NVL(AX.VALEVE,0)) 
          FROM FOL AX
         WHERE AX.INCIRM <> 'N'
           AND EXISTS(SELECT 1 FROM VETORH.R008EVB EVB WHERE EVB.CODTAB = 941 AND EVB.CODEVE = AX.CODEVE AND EVB.EVEBAS IN (9001,9002,9061,40))) AS BASEIR,
       FOL.faixaIR, fol.INCIRM, (SELECT COUNT(*) FROM VETORH.R008EVB EVB WHERE EVB.CODTAB = 941 AND EVB.CODEVE = FOL.CODEVE AND EVB.EVEBAS IN (9001,9002,9061,40)) AS CONTA
 FROM FOL
